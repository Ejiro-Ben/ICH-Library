...
export const PastQuestionsCard = ({ question }) => {
    ...
    const handleView = () => {
        ...
        const fileType = question.file_type?.toLowerCase() || '';
        
        // Check if file is viewable in browser (PDF, images)
        if (fileType === 'pdf' || fileType === 'image') {
            window.open(question.file_url, '_blank');
        } else {
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            let downloadName = question.file_name || 'file';
            ...
            <div className="bg-chem-cyan/10 border border-chem-cyan/10 p-3 rounded-lg flex w-full">
                ...
                <div>
                    <div className="flex justify-between">
                        <h1 className="text-white font-bold">{question.course_title}</h1>
                        <p className="text-chem-cyan font-bold border border-chem-cyan rounded-lg px-2 h-6">{getFileType(question.file_name)}</p>
                    </div>

                    <div className="flex space-x-2 mt-2">
                        <p className="bg-chem-green px-2 rounded-lg">{question.author}</p>
                        <p className="bg-chem-green px-2 rounded-lg">{question.course_code}</p>
                    </div>
